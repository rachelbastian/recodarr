...
import { SystemUsageTriggerConfig, TimeTriggerConfig, OnDemandTriggerConfig } from '../../config/triggerTypes';
...
export const TriggerConfigPanel: React.FC<TriggerConfigPanelProps> = ({ nodeId, triggerType }) => {
  const { nodes, setNodes } = useWorkflows();
  
  // Find the current node and load its configuration
  const node = nodes.find(n => n.id === nodeId);
  const config = node?.data?.triggerConfig;
  
  const updateConfig = (updates: Partial<SystemUsageTriggerConfig | TimeTriggerConfig | OnDemandTriggerConfig>) => {
    setNodes(nodes => 
      nodes.map(node => {
        if (node.id === nodeId) {
          return {
        ...
        <div className="p-4 space-y-4 max-w-full">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <Select 
                ...
                onValueChange={(value: string) => 
                  updateConfig({ resourceType: value as SystemUsageTriggerConfig['resourceType'] })
                }
              >
            ...
            <div>
              ...
              <Select 
                ...
                onValueChange={(value: string) => 
                  updateConfig({ operator: value as SystemUsageTriggerConfig['operator'] })
                }
              >
                <SelectTrigger className="w-full h-8 text-xs">
          ...
          <div className="space-y-2">
            ...
            <Slider 
              ...
              max={100}
              step={1}
              onValueChange={(value: number[]) => updateConfig({ threshold: value[0] })}
              className="py-2"
            />
          </div>
        ...
        <div className="p-4 space-y-4 max-w-full">
          ...
          <div className="space-y-2">
            ...
            <Select 
              ...
              onValueChange={(value: string) => 
                updateConfig({ schedule: value as TimeTriggerConfig['schedule'] })
              }
            >
              <SelectTrigger className="w-full h-8 text-xs">
        ...
        <div className="p-4 space-y-4 max-w-full">
          ...
          <div className="flex items-center space-x-2">
            <input 
              ...
              onChange={(e) => 
                updateConfig({ requireConfirmation: e.target.checked })
              }
              className="rounded border-zinc-500 h-3 w-3"
            />
    ...
    default:
      return (
        <div className="p-4 max-w-full">
          ...
          <p className="text-xs text-zinc-400">
            Trigger type "{config.type}" is not recognized.
          </p>
        </div>
      );
  }
